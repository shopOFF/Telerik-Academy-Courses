...
BEGIN 
	INSERT INTO People(FirstName, LastName, SSN)
		VALUES ('FirstName ' + CONVERT(VARCHAR(6), @counter),
				'LastName ' + CONVERT(VARCHAR(6), @counter), 
				 @counter + 12345)
...
BEGIN 
	INSERT INTO Accounts(Balance, PersonId)
		VALUES (@counter * 50, @counter)
		SET @counter = @counter - 1
END
GO

--- Creates stored procedure
CREATE PROC usp_SelectFullNameOfAllPeople
AS
	SELECT FirstName + ' ' + LastName[Full Name]
	FROM People 
GO

EXEC usp_SelectFullNameOfAllPeople;


--2. Create a stored procedure that accepts a number as a parameter and returns all persons who have more money in their accounts than the supplied number.

CREATE PROC usp_SelectAllPeopleWithMoney500OrMore(@minMoneyInAccount MONEY)
AS
...
EXEC usp_SelectAllPeopleWithMoney500OrMore 500;

--3. Create a function that accepts as parameters – sum, yearly interest rate and number of months.
	-- It should calculate and return the new sum.
	-- Write a SELECT to test whether the function works as expected.

CREATE FUNCTION dbo.ufn_CalculateBonus(@sum MONEY, @yearlyInterestRate FLOAT, @numOfMoths INT)
	RETURNS MONEY
AS
BEGIN
	RETURN @sum + (@yearlyInterestRate / 12) * @numOfMoths * @sum
END
GO

USE TelerikAcademy

SELECT Salary, dbo.ufn_CalculateBonus(Salary / 12, 0.1, DATEDIFF(month, HireDate, GETDATE()))[Bonus]
FROM Employees 
...
EXEC usp_DepositMoney 1, 2009

EXEC usp_WithdrawMoney 2, 399
